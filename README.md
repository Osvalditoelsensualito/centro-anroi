...
## Corrección #5 (sobre esta misma versión) — qué se implementó

### 1. Investigación real de por qué los Hábitos no notificaban fuera de la app

Volví a leer, línea por línea, el código fuente completo del plugin oficial
`@capacitor/local-notifications` (`LocalNotificationsPlugin.java`, `LocalNotificationManager.java`,
`LocalNotification.java`, `LocalNotificationSchedule.java`, `NotificationStorage.java`) y lo comparé
con el código de la app para Hábitos y para Recordatorios. Conclusión concreta: **ambos ya usaban
exactamente el mismo mecanismo nativo** — el mismo canal de notificaciones, el mismo
`AlarmManager.setExactAndAllowWhileIdle` (la llamada de Android que dispara alarmas incluso con la
pantalla apagada o en modo ahorro de batería), y la misma función `nativeSyncAllNotifications()`.
No era un problema "visual" ni de que Hábitos usara un sistema distinto.

Pero encontré un riesgo real y verificable en el propio código del plugin: la función
`LocalNotification.buildNotificationList()` (la que arma la lista antes de programar cualquier
notificación) **descarta el lote entero si un solo elemento del array falla al validarse** — antes,
hábitos, recordatorios, agua y tareas se enviaban todos juntos en una sola llamada
`ln.schedule({notifications:[...]})`. Si en algún momento un solo hábito (por ejemplo uno con un
horario mal guardado) fallaba esa validación, **ninguna notificación de ese lote se programaba, ni
siquiera las de Recordatorios que sí estaban bien formadas** — pero como Recordatorios normalmente
se reprograma en otro momento del ciclo de vida de la app, terminaba viéndose como si "solo los
hábitos" fallaran, cuando en realidad cualquier categoría podía verse afectada por errores de otra.

### 2. La corrección: cada categoría se programa por separado, hábitos primero

Ahora `nativeSyncAllNotifications()` arma cuatro listas independientes — hábitos, recordatorios,
agua y tareas — y llama a `ln.schedule()` **una vez por cada categoría**, en llamadas separadas
protegidas individualmente. Esto significa que un problema en una categoría ya **no puede bloquear
silenciosamente** a las demás, y los hábitos ahora se intentan programar primero. Antes de armar
cada notificación se valida su horario con una función nueva (`validSchedule`) que descarta
cualquier dato corrupto (hora fuera de 0-23, minuto fuera de 0-59, día de semana inválido, fecha
inválida) para que nunca llegue al plugin nativo un dato que pueda hacer fallar la validación.

Importante: esto **no es un sistema nuevo ni paralelo** — es exactamente el mismo mecanismo que ya
usa Recordatorios (mismo canal, mismo `AlarmManager`, misma función), solo que ahora cada categoría
se programa de forma aislada para que sea más robusto, tal como pediste ("no quiero dos sistemas
diferentes si no es necesario").

### 3. Herramientas nuevas para que vos mismo verifiques en tu teléfono

Como no tengo un dispositivo Android real en este entorno de trabajo para probar el comportamiento
en segundo plano, agregué en Ajustes dos cosas concretas para que la verificación real quede en tus
manos, tal como corresponde:

- Un contador de diagnóstico que muestra cuántas notificaciones nativas están pendientes en este
  momento y el resultado de la última sincronización, desglosado por categoría: "N de hábitos
  (✓/✕) · N de recordatorios (✓/✕) · N de agua", para que puedas confirmar de un vistazo que los
  hábitos sí se están programando.
- Un botón nuevo "🧪 Probar notificación de hábito (10 s)" que programa una notificación real con
  exactamente la misma forma que una notificación de hábito verdadera (mismo canal, mismos botones
  HECHO / RECUÉRDAME MÁS TARDE), para dentro de 10 segundos. La prueba real es: tocás el botón,
  cerrás la app (o apagás la pantalla, o abrís otra app), y a los 10 segundos tiene que aparecer la
  notificación en la barra de Android sin que hayas vuelto a abrir Centro.

### 4. Los botones HECHO / RECUÉRDAME MÁS TARDE no se tocaron

Los botones de acción de las notificaciones de hábitos siguen siendo exactamente los mismos que ya
funcionaban (mismo `actionTypeId`, mismo manejo en el listener de acciones); lo único que cambió es
que ahora llegan de forma más confiable porque ya no dependen de que el lote combinado se programe
sin errores.

### 5. Rediseño visual de Recordatorios (calendario)

Se rediseñó el calendario de la pantalla de Recordatorios siguiendo la imagen de referencia que
mandaste: tarjeta oscura tipo "cristal" con un resplandor violeta arriba, días en círculos en vez de
casilleros, el día de hoy resaltado con un círculo violeta relleno, y los días de los meses
anterior/siguiente que se ven al principio/final de la grilla ahora aparecen atenuados (en vez de no
mostrarse). Los días con recordatorios muestran un punto de color en vez del texto "N rec." (el
detalle completo sigue disponible al tocar el día, y también como tooltip). Este cambio es
**puramente visual**: se aplicó dentro de una clase nueva y aislada (`.reminder-cal-card`) para que
**no afecte** al calendario de Hábitos ni al del Diario de Trading, que comparten las mismas clases
base pero no debían cambiar. La función que decide qué recordatorio tiene cada día, y qué pasa al
tocarlo, es exactamente la misma de antes.

### 6. Nuevo diseño de anillos: calorías (Inicio y pestaña Calorías) y pasos

Se aplicó el nuevo estilo de gráfico circular (puntas redondeadas y separación entre segmentos, como
en tu imagen de referencia) a los tres gráficos que pediste, y a ningún otro:

- El anillo de calorías de la burbuja de Inicio.
- El anillo de calorías de la pestaña Calorías — que ahora **usa el mismo componente** que el de
  Inicio (`caloriesPie`), en vez de un componente distinto y más simple que tenía antes, tal como
  pediste para no duplicar código: son ahora visualmente idénticos porque son literalmente la misma
  función.
- El anillo de pasos de la pestaña Calorías.

Como aclaraste después, el cambio es **solo de forma**: los colores originales del gráfico de
calorías no se tocaron (verde para lo que resta, rojo para grasas, azul para proteínas, dorado para
carbohidratos) y el cálculo de cuánto arco dibuja cada segmento (la proporción real de lo consumido
contra la meta) tampoco cambió — lo único distinto es que ahora las puntas son redondeadas y hay un
pequeño espacio entre cada segmento, en vez de puntas semicirculares pegadas entre sí. Los anillos de
grasas/proteínas/carbohidratos (los tres círculos chicos) no se tocaron, siguen con su forma
original, porque no fueron parte de lo pedido.

### Pruebas hechas antes de entregar

- Se armó una simulación en Node.js con las funciones reales de generación de horarios
  (`habitNativeSchedules`, `reminderNativeSchedule`, `validSchedule`) usando datos de ejemplo
  realistas (hábitos diarios, semanales, con horarios normales y con datos corruptos a propósito)
  para confirmar que la nueva validación filtra correctamente los datos malos sin bloquear los
  buenos, y que la forma final que recibe el plugin nativo es idéntica entre hábitos y
  recordatorios.
- Se confirmó por lectura del código fuente del plugin que `registerActionTypes`/`getActionGroup`
  (los botones HECHO / RECUÉRDAME MÁS TARDE) nunca fallan ni devuelven nulo, y que
  `triggerScheduledNotification`/`setExactIfPossible` usan exactamente el mismo
  `AlarmManager.setExactAndAllowWhileIdle` para hábitos y recordatorios.
- Se verificaron con capturas de pantalla (Playwright), en tema claro y oscuro: el calendario nuevo
  de Recordatorios (con el día de hoy resaltado, los días de meses adyacentes atenuados, y el punto
  de un recordatorio real creado durante la prueba), el anillo de calorías con datos reales
  (1450 de 2200 kcal, grasa 55 g / proteína 90 g / carbohidratos 160 g) mostrando los colores
  originales con puntas redondeadas y separación, y el anillo de pasos con la punta redondeada.
- Se revisó que la pantalla de Inicio no tuviera regresiones: la burbuja de agua, la tarjeta de
  clima, la burbuja de actividad (pasos + calorías) y el menú inferior se ven igual que en la ronda
  anterior.
- Los 3 bloques de JavaScript de `index.html` siguen siendo sintácticamente válidos después de todos
  los cambios, y los archivos XML/Java del proyecto Android (que no se tocaron en esta ronda) se
  volvieron a validar sin errores.

### Nota sobre lo que no se pudo probar en un dispositivo físico

Este entorno de trabajo sigue sin tener un teléfono ni un emulador Android disponible, así que no
pude presionar el botón "🧪 Probar notificación de hábito", cerrar una app real y ver la notificación
aparecer en la barra de Android con mis propios ojos — eso es exactamente lo que te pido que hagas
vos con esta nueva versión, usando ese botón nuevo en Ajustes. Lo que sí pude hacer, y de forma
rigurosa: leer el código fuente completo del plugin nativo para confirmar que el mecanismo de
hábitos y recordatorios es matemáticamente el mismo (mismo canal, misma alarma exacta), identificar y
corregir un riesgo concreto de que un elemento con datos corruptos bloqueara el lote entero, y
dejarte dos herramientas de diagnóstico reales dentro de la app para que la verificación en tu
teléfono sea inmediata y no dependa de mi palabra. Si después de instalar esta versión programás un
hábito con horario cercano, cerrás la app, y la notificación NO aparece, avisame con el detalle
exacto (qué mostró el contador de diagnóstico de Ajustes en ese momento) y seguimos investigando
sobre datos reales de tu teléfono.
